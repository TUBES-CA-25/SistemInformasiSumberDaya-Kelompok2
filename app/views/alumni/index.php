<section class="resource-section">
    <div class="container">
        <div class="page-header">
            <h1>Jejaring Alumni</h1>
            <p>Daftar alumni asisten laboratorium yang telah berkontribusi dan kini berkarier di berbagai industri.</p>
        </div>

        <div class="alumni-group">
            <h3 class="section-divider">Angkatan 2023</h3>
            <div class="profile-grid">
                
                <a href="/detail-alumni.php?id=1" class="profile-card">
                    <div class="profile-img">
                        <img src="https://placehold.co/300x300/95a5a6/white?text=Alumni+1" alt="Foto">
                    </div>
                    <div class="profile-info">
                        <h3>Nama Alumni 1</h3>
                        <p>Ex-Koordinator Lab</p>
                        <small style="color: #666;">🚀 Software Engineer di Tokopedia</small>
                    </div>
                </a>

                <a href="/detail-alumni.php?id=2" class="profile-card">
                    <div class="profile-img">
                        <img src="https://placehold.co/300x300/95a5a6/white?text=Alumni+2" alt="Foto">
                    </div>
                    <div class="profile-info">
                        <h3>Nama Alumni 2</h3>
                        <p>Ex-Divisi Jaringan</p>
                        <small style="color: #666;">🌐 Network Engineer di Telkom</small>
                    </div>
                </a>

            </div>
        </div>

        <div class="alumni-group" style="margin-top: 60px;">
            <h3 class="section-divider">Angkatan 2022</h3>
            <div class="profile-grid">
                
                <a href="/detail-alumni.php?id=3" class="profile-card">
                    <div class="profile-img">
                        <img src="https://placehold.co/300x300/7f8c8d/white?text=Alumni+3" alt="Foto">
                    </div>
                    <div class="profile-info">
                        <h3>Nama Alumni 3</h3>
                        <p>Ex-Divisi Multimedia</p>
                        <small style="color: #666;">🎨 UI/UX Designer di Gojek</small>
                    </div>
                </a>

                <a href="/detail-alumni.php?id=4" class="profile-card">
                    <div class="profile-img">
                        <img src="https://placehold.co/300x300/7f8c8d/white?text=Alumni+4" alt="Foto">
                    </div>
                    <div class="profile-info">
                        <h3>Nama Alumni 4</h3>
                        <p>Ex-Koordinator Lab</p>
                        <small style="color: #666;">🏛️ ASN Kominfo</small>
                    </div>
                </a>

            </div>
        </div>
    </div>
</section>