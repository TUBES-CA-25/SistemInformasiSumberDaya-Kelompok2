<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    ...
    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
        <div class="relative w-full sm:w-64">
            ...
            <input type="text" id="searchInput" placeholder="Cari sanksi..." 
                   class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-shadow text-sm">
        </div>

        <button onclick="loadSanksi()" 
           class="bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-red-600 px-4 py-2.5 rounded-lg shadow-sm transition-all duration-200 flex items-center justify-center gap-2 font-medium">
            <i class="fas fa-sync-alt"></i>
        </button>

        <button onclick="openFormModal()" 
           class="bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-lg shadow-sm transition-all duration-200 flex items-center justify-center gap-2 font-medium transform hover:-translate-y-0.5 whitespace-nowrap">
...
<div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        <span class="text-sm text-gray-500 font-medium">Daftar Sanksi Pelanggaran</span>
        <span id="totalData" class="text-xs font-bold text-red-600 bg-red-50 px-3 py-1 rounded-full">Total: 0</span>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-800 text-white text-sm uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4 font-semibold w-1/4">Jenis Pelanggaran</th>
                    <th class="px-6 py-4 font-semibold">Sanksi / Hukuman</th>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                        <div class="flex flex-col items-center gap-2">
                            <i class="fas fa-circle-notch fa-spin text-red-500 text-2xl"></i>
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all w-full sm:max-w-lg border border-gray-100">
            ...
            <div class="p-6">
                <form id="sanksiForm">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Judul Pelanggaran <span class="text-red-500">*</span></label>
                            <input type="text" id="inputJudul" name="judul" required
                                   ...
                                   placeholder="Contoh: Terlambat Datang Praktikum">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi Sanksi <span class="text-red-500">*</span></label>
                            <textarea id="inputDeskripsi" name="deskripsi" rows="6" required
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-colors resize-none"
                                      placeholder="Jelaskan detail hukuman/sanksi yang diberikan..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-6 mt-2 border-t border-gray-100">
                        ...
                        <button type="submit" id="btnSave" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-colors flex items-center gap-2 shadow-sm">
...
<div id="detailModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    ...
    <div class="flex min-h-screen items-center justify-center p-4">
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all w-full sm:max-w-2xl border border-gray-100">
            ...
            <div class="bg-red-50 px-6 py-4 border-b border-red-100 flex justify-between items-center sticky top-0 z-10">
                <h3 class="text-xl font-bold text-red-800 flex items-center gap-2">
                    <i class="fas fa-info-circle"></i> Detail Sanksi
                </h3>
                <button onclick="closeModal('detailModal')" class="text-red-600 hover:text-red-800 transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
...
function renderTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        const row = `
            <tr onclick="openDetailModal(${item.id})" class="hover:bg-red-50 transition-colors duration-150 group border-b border-gray-100 cursor-pointer">
                <td class="px-6 py-4 text-center font-medium text-gray-500">${index + 1}</td>
                <td class="px-6 py-4">
                    <span class="font-bold text-gray-800 text-sm block group-hover:text-red-600 transition-colors">${escapeHtml(item.judul)}</span>
                </td>
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-center items-center gap-2">
                        ...
                        <button onclick="hapusSanksi(${item.id}, event)" class="w-9 h-9 rounded-lg bg-red-100 text-red-600 hover:bg-red-500 hover:text-white transition-all shadow-sm flex items-center justify-center border border-transparent" title="Hapus">
...
function openFormModal(id = null, event = null) {
    ...
    const form = document.getElementById('sanksiForm');
    ...
    if (id) {
        title.innerHTML = '<i class="fas fa-edit text-red-600"></i> Edit Sanksi';
        btnSave.innerHTML = '<i class="fas fa-save"></i> Update Data';
        const data = allSanksiData.find(i => i.id == id);
        if (data) {
        ...
        title.innerHTML = '<i class="fas fa-plus text-red-600"></i> Tambah Sanksi';
...
document.getElementById('sanksiForm').addEventListener('submit', function(e) {
    ...
    const url = id ? API_URL + '/sanksi-lab/' + id : API_URL + '/sanksi-lab';
...
function openDetailModal(id) {
    ...
    .then(res => {
        ...
        content.innerHTML = `
            <div>
                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Judul Pelanggaran</h4>
            ...
            <div class="border-t border-gray-100 pt-3">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-lg bg-red-50 text-red-700 text-sm font-medium">
                    <i class="far fa-calendar-alt mr-2"></i> ${tanggal}
                </span>
            </div>

            <div class="border-t border-gray-100 pt-3">
                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Deskripsi Sanksi</h4>
...
function filterTable(searchTerm) {
    ...
    const filtered = allSanksiData.filter(item => 
